...
def train_mnist(batch_size: int, learning_rate: float, epochs: int, activation_fn: int, cost_fn: int):
    """Train model on MNIST dataset"""

    activation_fn = {0: Sigmoid}[activation_fn]
    cost_fn = {0: QuadraticCost, 1: CrossEntropyCost}[cost_fn]
    

    layers = [784, 128, 64, 10]
    nw = Network(layers, activation_fn, cost_fn)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", type=int, default=0, help="Cost function (default: quadratic)")
    ...
    nw = train_mnist(batch_size = args.bs, learning_rate = args.lr, epochs = args.e, 
                     activation_fn = args.a, cost_fn = args.c)
